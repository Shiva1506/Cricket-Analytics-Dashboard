...
def load_all_data():
    batting_files = {
        'ODI': 'player_batting_stats_odi.csv',
        'T20': 'player_batting_stats_t20.csv',
        'Test': 'player_batting_stats_test.csv'
    }
    
    bowling_files = {
        'ODI': 'player_bowling_stats_odi.csv',
        'T20': 'player_bowling_stats_t20.csv',
        'Test': 'player_bowling_stats_test.csv'
    }
    ...
    for format_name, filename in bowling_files.items():
        try:
            if os.path.exists(filename):
                ...
                # Calculate missing bowling metrics
                if 'runs_conceded' in df.columns and 'wickets' in df.columns:
                    df['bowling_average'] = df['runs_conceded'] / df['wickets']
                    df['bowling_average'] = df['bowling_average'].replace([np.inf, -np.inf], 0).fillna(0)
                
                if 'balls_bowled' in df.columns and 'wickets' in df.columns:
                    df['bowling_strike_rate'] = df['balls_bowled'] / df['wickets']
                    df['bowling_strike_rate'] = df['bowling_strike_rate'].replace([np.inf, -np.inf], 0).fillna(0)
                
                if 'runs_conceded' in df.columns and 'balls_bowled' in df.columns:
                    df['economy'] = (df['runs_conceded'] / df['balls_bowled']) * 6
                    df['economy'] = df['economy'].replace([np.inf, -np.inf], 0).fillna(0)
    ...
    return batting_data, bowling_data, batsman_vs_bowler_data, playing_xi_data, match_data, ball_data

# Load all data
batting_data, bowling_data, batsman_vs_bowler_data, playing_xi_data, match_data, ball_data = load_all_data()
...
def predict_next_match_performance(player_name, role, opponent, venue, pitch_conditions, format_name=None):
    ...
    # Use selected_format if format_name is not provided
    if format_name is None:
        format_name = selected_format
    # This is a simplified prediction - in a real scenario, you'd use machine learning
    base_performance = {
...
def recommend_playing_xi(team_name, opponent, venue, pitch_conditions='normal', format_name=None):
    """Recommend optimal Playing XI based on multiple factors"""
    if format_name is None:
        raise ValueError("format_name must be provided to recommend_playing_xi")
...
if main_tab == "Team & Player Stats":
    stats_type = st.sidebar.radio("Select Statistics Type", ["Batting", "Bowling"])
...
if main_tab == "Team & Player Stats":
    ...
    if stats_type == "Batting":
        ...
        with col4:
            st.markdown('<div class="metric-card">', unsafe_allow_html=True)
            total_boundaries = filtered_df['fours'].sum() + filtered_df['sixes'].sum() if 'fours' in filtered_df.columns and 'sixes' in filtered_df.columns else 0
        ...
        with col4:
            st.markdown('<div class="metric-card bowling-metric">', unsafe_allow_html=True)
            avg_wickets = filtered_df['wickets'].mean()
            st.metric("Avg Wickets/Bowler", f"{avg_wickets:.1f}")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab1:
        if stats_type == "Batting":
            ...
            with col3:
                # Most boundaries
                if 'fours' in filtered_df.columns and 'sixes' in filtered_df.columns:
                    filtered_df['total_boundaries'] = filtered_df['fours'] + filtered_df['sixes']
                    top_boundaries = filtered_df.nlargest(10, 'total_boundaries')[['batsman', 'total_boundaries', 'fours', 'sixes']]
                    fig = px.bar(top_boundaries, x='total_boundaries', y='batsman', orientation='h',
                                title='Top 10 Boundary Hitters', color='total_boundaries',
            ...
            with col4:
                # Best strike rates
                min_wickets_for_sr = 10 if selected_format == 'T20' else 20
                qualified_bowlers = filtered_df[filtered_df['wickets'] >= min_wickets_for_sr]
                if not qualified_bowlers.empty and 'bowling_strike_rate' in qualified_bowlers.columns:
                    top_sr = qualified_bowlers.nsmallest(10, 'bowling_strike_rate')[['bowler', 'bowling_strike_rate', 'wickets', 'economy']]
                    fig = px.bar(top_sr, x='bowling_strike_rate', y='bowler', orientation='h',
                                title=f'Best Strike Rates (Min {min_wickets_for_sr} wickets)', color='bowling_strike_rate',
                                color_continuous_scale='Rainbow')
                    fig.update_layout(yaxis={'categoryorder':'total ascending'})
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    st.info(f"No bowlers with {min_wickets_for_sr}+ wickets found with current filters or bowling strike rate data not available")
    ...
    with tab2:
        if stats_type == "Batting":
            ...
            with col4:
                # Boundaries distribution if available
                if 'fours' in filtered_df.columns and 'sixes'in filtered_df.columns:
                    fig = px.scatter(filtered_df, x='fours', y='sixes', 
                                    color='strike_rate', size='runs',
                                    title='Fours vs Sixes (Size: Total Runs, Color: Strike Rate)',
                                    hover_data=['batsman'])
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    st.info("Boundary data not available")
    ...
    with tab3:
        if stats_type == "Batting":
            ...
            if 'fours' in filtered_df.columns:
                numeric_cols.append('fours')
            if 'sixes' in filtered_df.columns:
                numeric_cols.append('sixes')
            ...
            if 'maidens' in filtered_df.columns:
                numeric_cols.append('maidens')
    ...
    with tab4:
        if stats_type == "Batting":
            ...
            if 'fours' in filtered_df.columns and 'sixes' in filtered_df.columns:
                filtered_df['boundary_percentage'] = ((filtered_df['fours'] * 4 + filtered_df['sixes'] * 6) / filtered_df['runs'] * 100)
                filtered_df['boundary_percentage'] = filtered_df['boundary_percentage'].replace([np.inf, -np.inf], 0).fillna(0)
            
            if 'balls_faced' in filtered_df.columns and 'dismissals' in filtered_df.columns:
                filtered_df['balls_per_dismissal'] = filtered_df['balls_faced'] / filtered_df['dismissals'].replace(0, 1)
            ...
            with col2:
                ...
                if 'boundary_percentage' in filtered_df.columns:
                    min_runs_boundary = 200 if selected_format == 'T20' else 500
                    boundary_kings = filtered_df[(filtered_df['runs'] >=min_runs_boundary) & (filtered_df['boundary_percentage'] <= 100)]
                    if not boundary_kings.empty:
                        boundary_kings = boundary_kings.nlargest(15, 'boundary_percentage')
                        fig = px.bar(boundary_kings, x='boundary_percentage', y='batsman', orientation='h',
                                    title=f'Highest Boundary Percentage (Min {min_runs_boundary} runs)', color='boundary_percentage')
                        fig.update_layout(yaxis={'categoryorder':'total ascending'})
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        st.info(f"No players with {min_runs_boundary}+ runs found with current filters")
                else:
                    st.info("Boundary data not available")
            ...
            if 'balls_bowled' in filtered_df.columns:
                ...
                filtered_df['wickets_per极00_balls'] = filtered_df['wickets_per_100_balls'].replace([np.inf, -np.inf], 0).fillna(0)
        ...
        if stats_type == "Batting":
            ...
            if selected_players:
                ...
                with col1:
                    ...
                    if 'fours' in selected_data.columns:
                        metrics.append('fours')
                    if 'sixes' in selected_data.columns:
                ...
                with col2:
                    ...
                    if 'fours' in selected_data.columns:
                        display_cols.append('fours')
                    if 'sixes' in selected_data.columns:
                        display_cols.append('sixes')
                    if 'dismissals' in selected_data.columns:
                        display_cols.append('dismissals')
            ...
            if selected_players:
                ...
                with col2:
                    ...
                    if 'overs' in selected_data.columns:
                    ...
                    if 'maidens' in selected_data.columns:
                        display_cols.append('maidens')
    ...
    with tab6:
        if stats_type == "Batting":
            ...
            if len(batting_data) > 1:
                ...
                if 'fours' in combined_data.columns:
                    format_stats[('fours', 'mean')] = combined_data.groupby('format')['fours'].mean()
                if 'sixes' in combined_data.columns:
                    format_stats[('sixes', 'mean')] = combined_data.groupby('format')['sixes'].mean()
                ...
                col3, col极 = st.columns(2)
                
                with col3:
                    # Average boundaries by format if available
                    if 'fours_mean' in format_stats.columns and 'sixes_mean' in format_stats.columns:
                        format_stats['boundaries_per_match'] = (format_stats['fours_mean'] + format_stats['sixes_mean']) / format_stats['matches_mean']
                        fig = px.bar(format_stats, x='format',y='boundaries_per_match', 
                                    title='Average Boundaries per Match by Format', color='format',
                                    labels={'boundaries_per_match': 'Boundaries per Match', 'format': 'Format'})
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        st.info("Boundary data not available for format comparison")
            ...
            if len(bowling_data) > 1:
                ...
                with col4:
                    ...
                    fig = px.bar(format_stats, x='format', y='bowler_count', 
                                ...
                                labels={'bowler_count': 'Number of Bowlers', 'format': 'Format'})
                    st.plotly_chart(fig, use_container_width=True)
    ...
    if selected_batsmen:
        st.markdown('<h3 class极section-header">Batsmen Comparison</h3>', unsafe_allow_html=True)
    ...
    if batsman_vs_bowler_data:
        ...
        if bvb_format in batsman_vs_bowler_data:
            ...
            if not matchup_data.empty:
                ...
                with col4:
                    st.markdown('<div class="metric-card player极vs-player">', unsafe_allow_html=True)
        ...
        if trend_format in ball_data:
            ...
            if analysis_type == "Player Trends":
                ...
                if team_matches.empty:
                    ...
                    match_results['win'] = (match_results['result'] == 'Win').astype(int)
    ...
    if ball_data:
        ...
        if predictor_format in ball_data:
            # Player selection
            players = list(ball_data[predictor_format]['batsman'].unique()) + list(ball_data[predictor_format]['bowler'].unique())
            players = [p for p in players if pd.notna(p)]
            players = sorted(set(players))
            ...
            # Determine player role
            if selected_player in ball_data[predictor_format]['batsman'].values:
                role = 'batsman'
            else:
                role = 'bowler'
                    ...
                    with col2:
                        ...
                        if predictor_format in ball_data:
                            opponent_matches = ball_data[predictor_format][
                                ...
                                (ball_data[predictor_format]['team2' if role == 'batsman' else 'batting_team'] == opponent)
                            ]
...
if st.sidebar.checkbox("Show raw data"):
    if main_tab == "Team & Player Stats":
        ...
        if selected_batsmen:
            st.sidebar.dataframe(batting_data[comparison_format][batting_data[comparison_format]['batsman'].isin(selected_batsmen)])
        if selected_bowlers:
            st.sidebar.dataframe(bowling_data[comparison_format][bowling_data[comparison_format]['bowler'].isin(selected_bowlers)])
    elif main_tab == "Batsman vs Bowler":
        if batsman_vs_bowler_data and bvb_format in batsman_vs_bowler_data:
            st.sidebar.dataframe(batsman_vs_bowler_data[bvb_format])
    elif main_tab == "Team Selection Assistant":
        if playing_xi_data and team_format in playing_xi_data:
            st.sidebar.dataframe(playing_xi_data[team_format])
    elif main_tab == "Players by Team":
        if ball_data and team_format in ball_data:
            st.sidebar.dataframe(ball_data[team_format][['match_id', 'team1', 'team2', 'venue', 'date']].drop_duplicates().head(20))
    elif main_tab == "Match Analysis":
        if match_data and match_format in match_data:
            st.sidebar.dataframe(match_data[match_format])
    elif main_tab == "Ball-by-Ball Analysis":
        if ball_data and ball_format in ball_data:
            st.sidebar.dataframe(ball_data[ball_format][ball_data[ball_format]['match_id'] == selected_match].head(100))
